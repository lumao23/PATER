...
from util import LabelSmoothingCrossEntropy, RecorderMeter, RecorderMeter1, SAM, train_one_epoch, evaluate8, save_checkpoint
...
parser.add_argument('--dataset', default='AffectNet_8', type=str, help='dataset of AffectNet_8')
...
parser.add_argument('--evaluate', default=None, type=str, help='evaluate model on val set')
parser.add_argument('--beta', type=float, default=0.6, help='the param of the hyper.')
parser.add_argument('--gpu', type=str, default='0,1', help='the number of GPU for model train and validation.')
...
def main():
    ...
    # create model
    model = Model(num_classes=8, pretrained=True, drop_path_rate=0.25)
    ...
    if args.evaluate is None:
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset,
                                                   ...
                                                   pin_memory=True,
                                                   drop_last=True)
    val_dataset = datasets.ImageFolder(os.path.join(args.dataset, 'val'),
                                       transforms.Compose([transforms.Resize((224, 224)),
                                                           transforms.ToTensor(),
                                                           transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                                std=[0.229, 0.224, 0.225])]))
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, um_workers=args.workers, pin_memory=True)
    if args.evaluate is not None:
        ...
        evaluate8(model=model, data_loader=val_loader, args=args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_acc, val_los, output, labels, D = evaluate8(model, val_loader, args)
        ...
        recorder.plot_curve(os.path.join(args.log, args.dataset, curve_name))
