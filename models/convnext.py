...
class ConvNeXt(nn.Module):
    ...
    def forward_features(self, x):
        ...
        #     x = self.stages[i](x)
        # return self.norm(x3.mean([-2, -1]))  # global average pooling, (N, C, H, W) -> (N, C)

    def forward(self, x):
        x1, x2, x3 = self.forward_features(x)
        out = self.norm(x3.mean([-2, -1]))
        out = self.head(out)
        return x1, x2, x3, out
...
def convnext_base(pretrained=False, **kwargs):
    ...
    # for k in out_dict.keys():
    #     if out_dict[k].shape != model_dict[k].shape:
    #         print('layer not equal:', str(k))
    #         continue
    #     if k in model_dict:
    #         model_dict[k] = out_dict[k].clone().to(model_dict[k].device)
    if pretrained:
        checkpoint = torch.load('./pretrain/convnext_base.pth', map_location='cpu')['state_dict']
        ...
        #     if param not in ['head.weight', 'head.bias']:
